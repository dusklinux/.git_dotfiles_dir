...
select_packages() {
  ...
  # NOTE: We print UI to >&2 (stderr) so it doesn't get captured by mapfile
  log_task "Selection Mode"
  printf "${C_YELLOW}Do you want to install ALL %s optional packages? [y/N] ${C_RESET}" "${#AVAILABLE_PACKAGES[@]}" >&2
