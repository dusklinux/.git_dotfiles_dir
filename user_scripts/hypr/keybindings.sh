...
# --- CONFIGURATION ---
# The Rofi command.
# We will append '-format i' dynamically to get the index of the selection.
...
DATA=$(get_binds | awk -F'\t' -v cache="$KEYMAP_CACHE" '
...

    {
        ...
        # Submap Indicator (visually distinct)
        submap_str = ""
        if (submap != "" && submap != "global") {
            submap_str = "<span foreground=\"#fab387\">[" toupper(submap) "]</span> "
        }
        ...
        # - Increased padding to 35 for complex combos
        # - Added submap color
        printf "<b>%-35s</b>   %s<span foreground=\"#a6adc8\">%s</span>####%s####%s\n", display_key, submap_str, action, dispatcher, arg
    }
' | sort -u)
