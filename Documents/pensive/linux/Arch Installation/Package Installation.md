...
```
pacman -S --needed kitty zsh zsh-syntax-highlighting starship fastfetch bat eza fd tealdeer yazi zellij gum man-db ttyper tree fzf less ripgrep
```
...
```
pacman -S --needed git meson cmake clang python-pipx uv rq jq bc luarocks viu chafa ueberzugpp
```
...
```
pacman -S --needed btop nvtop inxi sysstat logrotate acpid tlp tlp-rdw thermald powertop ncdu gdu iotop iftop lshw wev pacman-contrib gnome-keyring libsecret yad dysk fwupd
```
